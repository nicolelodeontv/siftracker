...
import { Check, Copy, HelpCircle, Minus, Plus, RotateCcw, Settings2, TimerReset, X } from 'lucide-react'
...
export default function Page() {
  ...
  useEffect(() => {
    try { window.localStorage.setItem(RATE_STORAGE_KEY, JSON.stringify({ rates: savedRates })) } catch { /* Ignore unavailable local storage. */ }
  }, [savedRates])
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => { if (event.key === 'Escape') setConfirmReset(false) }
  ...
  async function saveRates() {
    ...
    setSavedRates({ ...rates }); setSettingsOpen(false); setFeedback('saved')
  }
  function resetRates() { const defaults = { ...DEFAULT_RATES }; setRates(defaults); setSavedRates(defaults); setFeedback('saved') }
  function performReset() { setValues({ ...EMPTY_VALUES }); setClockInTime(getCurrentClockIn()); setConfirmReset(false); setFeedback('reset') }
  function handleClockNow() { setClockInTime(philippineTime) }
    ...
    <main className="min-h-screen overflow-x-hidden bg-background text-foreground">
      ...
      <div className="relative mx-auto w-full max-w-6xl px-4 pb-24 sm:px-6 sm:pb-8 lg:px-8">
        <nav className={`flex min-h-14 items-center justify-between gap-4 border-b border-border/70 ${motion}`}>
          ...
          <div className="flex min-w-0 items-center gap-2 sm:gap-3"><div className="hidden text-right sm:block"><span className="block text-[8px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">PHT Now</span><time className="block font-mono text-[10px] font-bold tabular-nums">{philippineTime}</time></div><time className="whitespace-nowrap font-mono text-[9px] font-semibold tabular-nums sm:hidden">{philippineTime} PHT</time><button type="button" onClick={openQuickGuide} className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-2.5 py-1.5 text-[9px] font-bold shadow-sm transition hover:border-primary/40 hover:bg-accent" aria-label="Open Quick Guide"><HelpCircle className="size-3" /><span className="hidden sm:inline">Quick Guide</span></button><ThemeToggle /></div>
        </nav>
        ...
        <section id="live-status" aria-label="Live shift status" className={`${motion} sticky top-2 z-30 mt-2 rounded-xl border border-border bg-card/95 shadow-md backdrop-blur`}>
          <div className="grid grid-cols-2 gap-px overflow-hidden rounded-xl bg-border sm:grid-cols-4">
            <div className="min-w-0 bg-card px-2.5 py-2 sm:px-3"><span className="block text-[7px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Work</span><strong className="mt-0.5 block truncate font-mono text-[11px] font-bold tabular-nums sm:text-xs">{formatDuration(totalSeconds)}</strong></div>
            <div className="min-w-0 bg-card px-2.5 py-2 sm:px-3"><span className="block text-[7px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Break</span><strong className="mt-0.5 block font-mono text-[11px] font-bold tabular-nums sm:text-xs">01:00:00</strong></div>
            <div className="min-w-0 bg-card px-2.5 py-2 sm:px-3"><span className="block text-[7px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Clock Out</span><strong className="mt-0.5 block truncate font-mono text-[11px] font-bold tabular-nums sm:text-xs">{formatMilitaryTime(estimatedClockOutSeconds)}</strong></div>
            <div className="min-w-0 bg-primary px-2.5 py-2 text-primary-foreground sm:px-3"><span className="block text-[7px] font-bold uppercase tracking-[0.14em] opacity-70">Status</span><strong className="mt-0.5 block truncate font-mono text-[10px] font-bold tabular-nums sm:text-xs">{shiftStatus}</strong></div>
          </div>
        </section>
