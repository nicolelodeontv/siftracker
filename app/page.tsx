      ...
      <div className="relative mx-auto w-full max-w-6xl px-5 pb-6 sm:px-7 lg:px-8">
        ...
        <section id="calculator" aria-labelledby="calculator-heading" className="scroll-mt-16">
          ...
          <div className="grid gap-2.5 lg:grid-cols-2">
            {workloads.map((workload) => { const calculated = calculateValue(values[workload.id]); return <article key={workload.id} className="group rounded-xl border border-border bg-card/85 p-3.5 shadow-[0_8px_28px_var(--card-shadow)] backdrop-blur transition duration-200 hover:border-primary/30"><div className="flex items-start justify-between gap-3"><div className="flex items-start gap-2.5"><span className="mt-1.5 size-2 shrink-0 rounded-full" style={{ backgroundColor: workload.accent }} /><div><h3 className="text-sm font-semibold tracking-tight">{workload.label}</h3><p className="mt-0.5 text-[10px] text-muted-foreground">{workload.minutesPerUnit} minutes per {workload.unit.slice(0, -1)}</p></div></div><output aria-live="polite" className="font-mono text-[15px] font-bold tabular-nums" style={{ color: workload.accent }}>{formatDuration((calculated ?? 0) * workload.minutesPerUnit)}</output></div><label className="mt-3 block"><span className="mb-1 block text-[8px] font-bold uppercase tracking-[0.15em] text-muted-foreground">Number of {workload.unit}</span><div className="relative"><input type="text" inputMode="numeric" value={values[workload.id]} onChange={(event) => updateValue(workload.id, event.target.value)} aria-label={`${workload.label} ${workload.unit}`} placeholder="0" className="h-11 w-full rounded-lg border border-input bg-background/70 px-3 pr-14 font-mono text-[15px] font-medium tabular-nums outline-none transition placeholder:text-muted-foreground/40 focus:border-primary focus:ring-4 focus:ring-primary/10" />{calculated !== null && <output className="pointer-events-none absolute inset-y-0 right-3 flex items-center font-mono text-[15px] font-semibold tabular-nums text-muted-foreground" aria-label="Calculated result">{calculated}</output>}</div></label><div className="mt-2.5 grid grid-cols-2 gap-x-2 gap-y-1 border-t border-border pt-2.5 sm:grid-cols-4">{workload.examples.map((example) => <span key={example} className="font-mono text-[8px] font-semibold leading-3.5 tracking-tight text-muted-foreground transition group-hover:text-foreground">{example}</span>)}</div></article> })}

            <section id="workflow" className="scroll-mt-16 rounded-xl border border-primary/15 bg-primary p-3.5 text-primary-foreground shadow-[0_8px_28px_var(--card-shadow)]"><div className="mx-auto grid w-full max-w-[620px] grid-cols-[minmax(0,1fr)_150px] items-center gap-4"><div className="min-w-0"><p className="text-[8px] font-bold uppercase tracking-[0.18em] opacity-70">02 / Workflow</p><h3 className="mt-1 text-base font-semibold tracking-tight">One total, all workloads.</h3><p className="mt-1 text-[10px] leading-4.5 opacity-75">The combined estimate updates automatically as you enter quantities above.</p></div><div className="w-[150px] min-w-[150px] text-right"><span className="block text-[8px] font-bold uppercase tracking-[0.16em] opacity-60">Combined total</span><strong className="mt-0.5 block whitespace-nowrap font-mono text-2xl font-bold tracking-[-0.04em] tabular-nums sm:text-3xl">{formatDuration(totalMinutes)}</strong></div></div></section>
          </div>
        </section>

        <section id="about" className="mt-5 scroll-mt-16 border-y border-border py-4 sm:flex sm:items-center sm:justify-between sm:gap-5"><div><p className="text-[8px] font-bold uppercase tracking-[0.18em] text-muted-foreground">03 / About</p><p className="mt-1 max-w-xl text-[10px] leading-4 text-muted-foreground"><span className="block">Built as a simple internal production planning tool.</span><span className="block">Type numbers or expressions such as <span className="font-mono text-foreground">4*3</span> directly into any field.</span></p></div><button type="button" onClick={reset} className="mt-3 inline-flex shrink-0 items-center justify-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-[10px] font-semibold transition hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:mt-0"><RotateCcw className="size-3" aria-hidden="true" />Reset all<ArrowUpRight className="size-3 opacity-50" aria-hidden="true" /></button></section>
