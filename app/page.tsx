      ...
      <div className="relative mx-auto w-full max-w-6xl px-5 pb-6 sm:px-7 lg:px-8">
        ...
        <section id="calculator" aria-labelledby="calculator-heading" className="scroll-mt-16">
          ...
          <div className="grid gap-2.5 lg:grid-cols-2">
            ...
            <section id="workflow" className="scroll-mt-16 rounded-xl border border-primary/15 bg-primary p-4 text-primary-foreground shadow-[0_12px_40px_var(--primary-shadow)] sm:p-5"><div className="mx-auto grid w-full max-w-[620px] grid-cols-[minmax(0,1fr)_150px] items-center gap-4"><div className="min-w-0"><p className="text-[8px] font-bold uppercase tracking-[0.18em] opacity-70">02 / Workflow</p><h3 className="mt-1 text-base font-semibold tracking-tight">One total, all workloads.</h3><p className="mt-1 text-[10px] leading-4.5 opacity-75">The combined estimate updates automatically as you enter quantities above.</p></div><div className="w-[150px] min-w-[150px] text-right"><span className="block text-[8px] font-bold uppercase tracking-[0.16em] opacity-60">Combined total</span><strong className="mt-0.5 block whitespace-nowrap font-mono text-2xl font-bold tracking-[-0.04em] tabular-nums sm:text-3xl">{formatDuration(totalMinutes)}</strong></div></div></section>
            <div className="lg:col-span-2 flex justify-end pt-1"><button type="button" onClick={reset} className="inline-flex items-center justify-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-[10px] font-semibold transition hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><RotateCcw className="size-3" aria-hidden="true" />Reset all<ArrowUpRight className="size-3 opacity-50" aria-hidden="true" /></button></div>
          </div>
        </section>

        <section id="about" className="mt-5 scroll-mt-16 border-y border-border py-4"><div><p className="text-[8px] font-bold uppercase tracking-[0.18em] text-muted-foreground">03 / About</p><p className="mt-1 max-w-xl text-[10px] leading-4 text-muted-foreground"><span className="block">Built as a simple internal production planning tool.</span><span className="block">Type numbers or expressions such as <span className="font-mono text-foreground">4*3</span> directly into any field.</span></p></div></section>
