...
export function ClockInPicker() {
  ...
  const adjust = (setter: (value: number) => void, value: number, delta: number, max: number) => {
        ...
        <div className="px-4 py-4 sm:px-5">
          ...
          <div className="mt-4 grid grid-cols-3 gap-2">
            {[
              ...
              <div key={item.label} className="min-w-0 rounded-xl border border-border bg-background/45 p-2.5">
                <span className="block text-center text-[8px] font-bold uppercase tracking-[0.16em] text-muted-foreground">{item.label}</span>
                <button type="button" onClick={() => adjust(item.set, item.value, 1, item.max)} className="mt-2 flex h-9 w-full items-center justify-center rounded-lg border border-border bg-card text-muted-foreground hover:bg-accent hover:text-foreground" aria-label={`Increase ${item.label}`}><Plus className="size-3.5" /></button>
                <button type="button" onClick={() => item.set( item.value )} className="mt-1.5 flex h-14 w-full items-center justify-center rounded-lg bg-primary/10 font-mono text-2xl font-bold tabular-nums text-primary">{pad(item.value)}</button>
                <button type="button" onClick={() => adjust(item.set, item.value, -1, item.max)} className="mt-1.5 flex h-9 w-full items-center justify-center rounded-lg border border-border bg-card text-muted-foreground hover:bg-accent hover:text-foreground" aria-label={`Decrease ${item.label}`}><Minus className="size-3.5" /></button>
              </div>
            ))}
          </div>
