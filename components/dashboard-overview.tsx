  ...
  return (
    <section id="dashboard" aria-label="SIF dashboard" className={`${cardClass} mb-4 overflow-hidden`}>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <div className="mt-4 flex flex-wrap items-end gap-x-4 gap-y-2">
              <div>
                <span className={`inline-flex rounded-full px-2.5 py-1 text-[8px] font-bold uppercase tracking-[0.13em] ${statusClass}`}>
                  {status}
                </span>
              ...
              <div>
                ...
                <strong className="mt-0.5 block font-mono text-xl font-bold tabular-nums">{formatDuration(shift.elapsedShiftSeconds)}</strong>
              </div>
              <div>
                <span className="block text-[7px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Time remaining</span>
                <strong className="mt-0.5 block font-mono text-xl font-bold tabular-nums text-primary">{shift.estimatedClockOutSeconds === null ? '—' : formatDuration(shift.timeLeftSeconds)}</strong>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 border-b border-border sm:grid-cols-4">
        <DashboardMetric icon={<Activity className="size-3" />} label="Active workloads" value={`${active}/${workloads.length}`} note={`${totalUnits} total units`} />
      ...
      <div className="grid gap-0 lg:grid-cols-[1.35fr_.65fr]">
              ...
              return (
                <div key={workload.id} className="min-w-0">
                  ...
                  <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-muted">
                    <div className="h-full rounded-full bg-primary/70 transition-[width] duration-300" style={{ width: `${share}%` }} />
                  </div>
                </div>
              )
            }) : (
        ...
        <div className="min-w-0 p-4 sm:p-5">
          ...
          <div className="mt-5 space-y-4">
            ...
            <TimelineItem label="Work complete" value={shift.clockInSeconds === null ? '—' : formatMilitaryTime(breakStartSeconds)} />
            <TimelineItem label="Break" value={shift.clockInSeconds === null ? '—' : `${formatMilitaryTime(breakStartSeconds)} → ${formatMilitaryTime(breakEndSeconds)}`} breakPoint />
