...
function TimelineItem({ label, value, color, active }: { label: string; value: string; color: string; active?: boolean }) { return <div className="relative flex items-start gap-3 rounded-lg border px-2 py-1.5" style={{ borderColor: `color-mix(in srgb, ${color} 20%, transparent)`, backgroundColor: `color-mix(in srgb, ${color} 7%, transparent)` }}><div className="relative z-10 flex w-3 shrink-0 justify-center"><span className="mt-1.5 size-2.5 rounded-full border-2 border-card ring-1" style={{ backgroundColor: color, borderColor: 'var(--card)', boxShadow: `0 0 0 1px ${color}, 0 0 10px color-mix(in srgb, ${color} 20%, transparent)` }} /></div><div className="min-w-0 flex-1"><span className="block text-[7px] font-bold uppercase tracking-[0.14em]" style={{ color }}>{label}</span><strong className={`mt-0.5 block truncate font-mono text-[11px] font-bold tabular-nums ${active ? '' : 'text-muted-foreground'}`} style={active ? { color } : undefined}>{value}</strong></div></div> }
